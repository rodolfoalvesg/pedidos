...
import (
	...
	"order-api/app/gateways/http"
	"order-api/app/gateways/http/client"
	"order-api/app/gateways/repository/db/postgres/migration"
...
func main() {
	...
	// Start Client Web
	client := client.NewUserClient(cfg)

	// Start the API server.
	srv, err := http.NewServer(ctx, &cfg, db, rdb, es, client)
	if err != nil {
